...
pub fn deinit(self: Rss, gpa: std.mem.Allocator) void {
...
fn checkFallible(
    ...
    if (status == .not_modified) {
        log.debug("RSS feed not modified: {f}", .{uri});
        return &.{};
    }
    ...
    while (root_it.next()) |ch| {
        if (!std.mem.eql(u8, ch.name(), "channel")) continue;

        var channel_it = ch.childrenIterator();
        while (channel_it.next()) |el| {
            var feed_title: ?[]const u8 = null;
            if (std.mem.eql(u8, el.name(), "title")) {
                feed_title = el.text();
            } else if (std.mem.eql(u8, el.name(), "item")) {
                ...
                while (property_it.next()) |property| {
                    if (std.mem.eql(u8, property.name(), "title")) {
                        ...
                        link = property.text();
                    } else if (std.mem.eql(u8, property.name(), "pubDate")) {
                        pub_date = time.parseIMF(property.text()) catch 0;
                    }
                }

                if (title == null or pub_date == 0) {
                    log.info("skipping item for {s}", .{self.spec.url});
                    continue;
                }

                if (pub_date <= state.last_modified) continue;
                if (pub_date > last_modified) last_modified = pub_date;

                // this is freshly configured feed, notify just one item, but keep searching for latest pub_date
                ...
                try notifications.append(gpa, .{
                    .title = if (title_override) |to|
                        ...
                        try gpa.dupeZ(u8, "RSS feed updated"),

                    .body = try gpa.dupeZ(u8, title.?),
